...
> Documentation en anglais : [architecture.md](architecture.md)
...
- **`Module.onEnable` qui jette** : le module est ignoré, log d'erreur, le
  serveur démarre. Un module cassé ne tue jamais le serveur. Les services
  qu'il a posés avant de jeter restent dans le kernel (pas de rollback) et
  réveillent leurs dépendants comme n'importe quel autre service.
